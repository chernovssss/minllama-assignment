...
def generate_sentence(args, prefix, outfile, max_new_tokens=75, temperature=0.0):
    with torch.no_grad():
        device = torch.device("cuda") if args.use_gpu else torch.device("cpu")
        ctx = torch.amp.autocast(device_type="cuda", dtype=torch.float32) if args.use_gpu else nullcontext()  # type: ignore
...
def test_with_prompting(args):
    ...
    with torch.no_grad():
        ...
        model = LlamaZeroShotClassifier(config, tokenizer, label_names)  # type: ignore
        model = model.to(device)
        ...
        write_predictions_to_file("dev", args.dev_out, dev_acc, dev_pred, dev_sents)  # type: ignore
        write_predictions_to_file("test", args.test_out, test_acc, test_pred, test_sents)  # type: ignore
...
def test(args):
    ...
    with torch.no_grad():
        ...
        write_predictions_to_file(
            "test", args.test_out, test_acc, test_pred, test_sents  # type: ignore
        )
